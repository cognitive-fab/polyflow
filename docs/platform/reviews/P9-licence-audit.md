...
| L7b | note | Python: the `langgraph` extra (P10) also pulls `certifi` (MPL-2.0), through httpx. A LangGraph-only install also installs `temporalio`, though the binding never imports it. | MPL-2.0 is file-level copyleft on certifi's own files only; nothing here modifies them. Still, the statement "MPL confined to openai-agents" no longer holds. | Name both extras in the audit (done here). Split the kernel into its own package, so the LangGraph extra carries no Temporal dependency (follow-up). |
