...
| P11 Temporal samples | **Phases 0–2 built.** `examples/upstream/` holds the pinned sources and `check.mjs`. (1) `python/examples/temporal_samples/customer_service`: the sample's workflow (Update handler, validator, continue-as-new) is byte-identical; its two inline tools moved to an in-process MCP server; the worker gained the plugin line. (2) `examples/temporal_samples/expense`: `activities.ts` byte-identical (Node 24 type stripping), the control flow a certified machine; admission refuses the mutation that pays without asking; the approval is an order addressed to the `human` role; nine scenarios in `sample-expense.test.mjs`. Found and fixed on the way: an Update handler may schedule an effect before the workflow function runs (the chain is created on first use from `Info.headers`); an effect after this execution's closure (TMPRL1102) was dropped from the record, now refused in both plugins; a proposal could complete a worker-performed order by hand (`outOfBand` now refuses it). **Not built:** phase 3 (`saga`). | [`P11-phase1-review.md`](reviews/P11-phase1-review.md): 0 blockers, 2 majors (fixed), 10 minors (6 fixed, 4 open by design or carried) |
