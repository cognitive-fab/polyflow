...
| P11 Temporal samples | **Phase 0 and 1 built.** `examples/upstream/` holds the pinned sources and `check.mjs`. `python/examples/temporal_samples/customer_service`: the sample's workflow (Update handler, validator, continue-as-new) is byte-identical; its two inline tools moved to an in-process MCP server; the worker gained one plugin line. Found and fixed on the way: an Update handler may schedule an effect before the workflow function runs (the chain is now created on first use, and a continued execution's handlers wait for its head), and an effect scheduled after this execution's closure (Temporal's TMPRL1102 case) was dropped from the record: both plugins now refuse it. **Not built:** phases 2 (`expense`) and 3 (`saga`). | this phase's review: pending |
