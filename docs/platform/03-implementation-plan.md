...
| P10 (optional) | A second engine binding at G0/G1 (Restate, DBOS or LangGraph), ledger and guard only. **Built: LangGraph** (`python/polyflow_langgraph`, see [`research/06-second-engine-langgraph.md`](research/06-second-engine-langgraph.md)) | The same conformance corpus passes on the second engine: `guard.json` and `routed-guard.json` through the binding's guard path, and an unmodified ReAct agent's ledger passes `polyflow verify` across a checkpoint resume |
| P11 | Official Temporal samples rewritten on Polyflow, the agent and workflow code kept byte-identical where the seam allows (checked by `examples/upstream/check.mjs` against pinned copies). Three picks: (1) samples-python `openai_agents/customer_service` at G1 (Python), (2) samples-typescript `expense` at G2 admitted at G3, (3) samples-typescript `saga` at G2 with a v2 through `vet` and the gate, no `patched()`. | (1) the workflow file identical; a seat change before a lookup refused with a witness, one change per conversation, an undeclared tool refused; the ledger verifies under the TS CLI. (2) admission refuses the machine with the approval acceptor deleted; no `payment` without a prior approve; duplicate start attaches. (3) `vet` upgrades or migrates every fleet state; an incompatible v3 is pinned; one continue-as-new per migrated run. |
...
| P10 Second engine (optional) | **Built at G0/G1 on LangGraph.** `govern(tools, ...)` wraps the `ToolNode` seam (`wrap_tool_call`). An unmodified `create_react_agent` is recorded and guarded with the same ledger, guard and sinks, and imports no `temporalio` (the sinks moved to `polyflow_temporal/sinks.py`). A thread's chain and guard state live in a per-thread record beside the sink, never in the agent's messages (P10 review). Each model turn is decided once under the thread's lock and bound to its exact calls, so resumes, replays, forks, trimming and parallel subagents continue one chain and charge once. The ledger verifies under the TypeScript CLI. **Not guaranteed** (LangGraph is not Temporal): at-most-once tool execution, and determinism of the agent's own code. **Not built:** escalation through `interrupt()`, G2/G3, sealed headers, a separate kernel distribution, a multi-host store, `polyflow verify --thread` in TypeScript (the check exists in Python). | [`P10-review.md`](reviews/P10-review.md): 2 blockers and 6 majors, all fixed |
| P11 Temporal samples | **Built (phases 0–3).** `examples/upstream/` holds the pinned sources and `check.mjs`, which asserts per port which files stayed byte-identical. (1) `python/examples/temporal_samples/customer_service` (G1, Python): the sample's workflow (Update handler, validator, continue-as-new) is byte-identical; its two inline tools moved to an in-process MCP server; the worker gained the plugin line. (2) `examples/temporal_samples/expense` (G2, admitted): `activities.ts` byte-identical (Node 24 type stripping), the control flow a certified machine; admission refuses the mutation that pays without asking; the approval is an order addressed to the `human` role; nine scenarios. (3) `examples/temporal_samples/saga` (G2 + G3 gate): activities, clients and types byte-identical (a loader hook for ts-node's extensionless imports); compensation discipline as certified guarantees (incl. completeness and completion-needs-its-order); STOP where nothing is to undo, CANCEL that compensates; v2 adds a KYC step and its compensation, `vet` over a fleet in every v1 state pins none, an incompatible v3 is pinned and the gate refuses it, and a run parked on v1 continues as new onto v2 under Worker Versioning with no `patched()`, a behavioural migration stated as such; ten scenarios. Found and fixed on the way: an Update handler may schedule an effect before the workflow function runs (the chain is created on first use from `Info.headers`); an effect after this execution's closure (TMPRL1102) was dropped from the record, now refused in both plugins; a proposal could complete a worker-performed order by hand (`outOfBand` refuses it); `vet` pinned every version that edits the mapper because polyvers defers its parent/child gates, which this host (no child machines) cannot need; a hand-over called in-flight orders off without waiting (TRY_CANCEL) and re-issued them on the new version, doing a step twice: orders now use WAIT_CANCELLATION_COMPLETED, and a run that moved on stays put with `migration-stale`. | [`P11-phase1-review.md`](reviews/P11-phase1-review.md): 0 blockers, 2 majors (fixed), 10 minors (6 fixed, 4 open by design or carried). [`P11-phase2-review.md`](reviews/P11-phase2-review.md): 1 blocker (fixed: the request survives restarts), 7 majors (6 fixed: the approver is in the journal and the ledger, timeouts, failure is not a rejection, axios/express, `unstoppable-states-refuse-stop` in admission; 1 documented: a parked slot per approval), 11 minors (6 fixed, 5 accepted or carried). [`P11-phase3-review.md`](reviews/P11-phase3-review.md): 1 blocker (fixed in the host: a hand-over waits for the step in flight, nothing is done twice), 5 majors (fixed: compensation completeness and completion-needs-its-order guarantees, STOP/CANCEL, vet honesty, `patched()` honesty, one chain across the hand-over), minors (mostly fixed; `payloadSchema` unread carried) |
