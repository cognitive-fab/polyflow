// customer-brief v2 — adds CANCEL: a run may be called off before anything is posted.
  ...
  component: {
    ...
    acceptors: {
      ...
      TICKETS_FAILED: (model) => (proposal, { reject, next, unchanged }) => {
        // The feed failed while gathering, or the draft failed while drafting
        // (draft_brief's failure is wired here): either way the brief failed.
        // Refusing it in drafting stranded the run (found by admission, P9 review WT1).
        if (!['gathering', 'drafting'].includes(model.briefState)) return reject('stale-completion');
      ...
      CANCEL: (model) => (proposal, { reject, next, unchanged }) => {
        if (!['gathering', 'drafting', 'review'].includes(model.briefState)) return reject('nothing-to-cancel');
        next.briefState = 'cancelled';
        next.reason = 'cancelled';
        unchanged('ticketCount');
      },
      STOP: (model) => (proposal, { reject, next, unchanged }) => {
        // FR-HUM.6: a person can always stop the run — except while the post
        // is in flight, which stopping would not unsend (declared unstoppable).
        if (!['idle', 'gathering', 'drafting', 'review'].includes(model.briefState)) return reject('nothing-to-stop');
        next.briefState = 'stopped';
        next.reason = 'stopped';
        unchanged('ticketCount');
      },
