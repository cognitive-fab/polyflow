...
module.exports.effects = function effects(pre, action, data, post, stepKind) {
  ...
  // v2: the KYC check, after the bank account.
  if (entered('adding_kyc')) out.push({ kind: 'addKycCheck', payload: withThrow(p, 'addKycCheck', { accountId: p.accountId, clientEmail: p.clientEmail }) });
  // Compensations, last first.
  if (entered('undo_bank')) out.push({ kind: 'disconnectBankAccounts', payload: { accountId: p.accountId } });
