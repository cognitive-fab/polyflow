// saga v2 — adds a KYC check after the bank account (a new step, a new
// compensation), the version change the gate promotes without patched().
// SAM v2 strict-profile module: Temporal's `saga` sample (openAccount) as a machine. Four forward steps in sequence; a failure compensates the
...
const bank = forward('adding_bank', 'addBankAccount', 'adding_kyc', 'undo_client');
const kyc = forward('adding_kyc', 'addKycCheck', 'opened', 'undo_bank');
const undoBank = compensation('undo_bank', 'undo_client');
const undoClient = compensation('undo_client', 'undo_address');
const undoAddress = compensation('undo_address', 'compensated');

const CANCEL_TO = { adding_client: { phase: 'undo_address', step: 'addClient' }, adding_bank: { phase: 'undo_client', step: 'addBankAccount' }, adding_kyc: { phase: 'undo_bank', step: 'addKycCheck' } };
const FAIL_AT = ['createAccount', 'addAddress', 'addClient', 'addBankAccount', 'addKycCheck'];
...
const control = instance({
  ...
  component: {
    ...
    actions: {
      START: { action: action(), schema: {}, domain: [{ ...COMMAND, failAt: '' }, { ...COMMAND, failAt: 'addBankAccount' }, { ...COMMAND, failAt: 'addKycCheck' }] },
      ...
      KYC_PASSED: { action: action(), schema: {}, domain: [{}] },
      KYC_FAILED: { action: action(), schema: {}, domain: [{ reason: 'step-failed' }] },
      BANK_DISCONNECTED: { action: action(), schema: {}, domain: [{}] },
      BANK_DISCONNECT_FAILED: { action: action(), schema: {}, domain: [{ reason: 'compensation-failed' }] },
    ...
    acceptors: {
      ...
      BANK_DISCONNECTED: undoBank.ok,
      BANK_DISCONNECT_FAILED: undoBank.failed,
